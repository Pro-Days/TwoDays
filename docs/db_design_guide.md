...
- **Projection**: `Keys Only` (결과로 기본 테이블의 `PK`인 `USER#<UUID>`를 획득)

## 5. 설계 장단점 요약

### 👍 장점
- **읽기 비용($RCU$) 극최적화**: 인덱스에 필요한 속성이 모두 포함되어 있어 베이스 테이블 추가 조회(`GetItem`)가 필요 없습니다.
- **무한한 확장성**: 데이터가 존재하는 한 1,000위, 10,000위 등 심해 랭킹까지 무한 페이지네이션 조회가 가능합니다.
- **날짜별 기록 보존**: 특정 과거 시점의 순위를 즉시 조회할 수 있습니다.
- **유연한 히스토리 추적**: 유저별 파티셔닝 덕분에 개별 유저의 성장을 트래킹하기 매우 용이합니다.

### 👎 단점
- **쓰기 비용($WCU$)**: 닉네임이 변경되거나 매일 스냅샷을 찍을 때 4개의 인덱스를 업데이트해야 하므로 쓰기 비용이 발생합니다.
- **해결책**: 일반적인 게임 서비스는 읽기 비중이 쓰기보다 압도적으로 높으므로, 전체적인 비용과 성능 면에서 이득입니다.
